...
public class OutboxPublisher {
    ...
    private void publish(OutboxEventEntity event, PublicationClock clock) {
        ...
        try {
            ...
            log.debug("Published notification outbox event {} type={}", event.getId(), event.getEventType());
    ...
    private void markFailed(OutboxEventEntity event, String message, java.time.Instant failedAt, boolean permanentFailure) {
        if (!retryProperties.writeStateEnabled()) {
            ...
            outboxRepository.save(event);
            log.warn("Failed to publish notification outbox event {} while retry-state writes are disabled: {}",
                    event.getId(), message);
            return;
        }
        ...
        if (event.getQuarantinedAt() != null) {
            log.error("Quarantined notification outbox event {} after {} attempts: {}",
                    event.getId(), event.getPublishAttempts(), message);
        } else {
            log.warn("Failed to publish notification outbox event {} (retry at {}): {}",

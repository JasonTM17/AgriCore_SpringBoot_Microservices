...
@ActiveProfiles("test")
class CropCycleErrorBoundaryIntegrationTest {
    ...
    void malformedJson_returnsStableBadRequest() throws Exception {
        assertApiError(
                mockMvc.perform(authenticated(post("/api/v1/crop-cycles"))
    ...
    void invalidPathIdentifier_returnsStableBadRequest() throws Exception {
        assertApiError(
                mockMvc.perform(authenticated(get("/api/v1/crop-cycles/not-a-uuid"))),
                HttpStatus.BAD_REQUEST,
                "INVALID_ARGUMENT"
        );
    }
    ...
    void invalidPaging_returnsValidationError(String query) throws Exception {
        assertApiError(
                mockMvc.perform(authenticated(get("/api/v1/crop-cycles" + query))),
                HttpStatus.BAD_REQUEST,
                "VALIDATION_FAILED"
        );
    }

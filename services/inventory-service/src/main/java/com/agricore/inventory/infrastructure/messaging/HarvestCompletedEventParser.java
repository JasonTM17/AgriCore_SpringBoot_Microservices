...
final class HarvestCompletedEventParser {
    ...
    Optional<HarvestCompletedCommand> parse(String raw) {
        ...
        UUID farmId = requiredUuid(payload, "farmId");
        requiredUuid(payload, "cropCycleId");
        ...
        requiredDate(payload, "harvestDate");
        requiredText(payload, "productName", 200);
        optionalText(payload, "farmName", 200);
        optionalText(payload, "plotCode", 64);
        optionalText(payload, "careSummary", 1000);

        return Optional.of(new HarvestCompletedCommand(
    ...
    private static void optionalText(JsonNode payload, String field, int maxLength) {
        JsonNode value = payload.get(field);
        if (value != null && (!value.isTextual() || value.textValue().isBlank())) {
            throw invalid("payload." + field + " must be a non-blank string when present");
        }
        if (value != null && value.textValue().length() > maxLength) {
            throw invalid("payload." + field + " must be at most " + maxLength + " characters");
        }
    }

...
public class OutboxPublisher {
    ...
    private void publish(OutboxEventEntity event, PublicationClock clock) {
        ...
        try {
            ...
            log.debug("Published inventory outbox event {} type={}", event.getId(), event.getEventType());
    ...
    private void markFailed(
        ...
        if (!retryProperties.writeStateEnabled()) {
            ...
            outboxRepository.save(event);
            log.warn("Failed to publish inventory outbox event {} while retry-state writes are disabled: {}",
                    event.getId(), message);
            return;
        }
        ...
        if (event.getQuarantinedAt() != null) {
            log.error("Quarantined inventory outbox event {} after {} attempts: {}",
                    event.getId(), event.getPublishAttempts(), message);
        } else {
            log.warn("Failed to publish inventory outbox event {} (retry at {}): {}",

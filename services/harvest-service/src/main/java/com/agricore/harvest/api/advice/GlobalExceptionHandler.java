package com.agricore.harvest.api.advice;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handle(HarvestException ex, HttpServletRequest request) {
    ...
    /**
     * A losing race on a unique index is the caller's conflict, not a server fault. The write paths
     * check for an existing row first, but that check is not a lock: two requests can both pass it
     * and the database rejects the second.
     *

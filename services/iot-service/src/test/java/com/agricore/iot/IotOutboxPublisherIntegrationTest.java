...
import com.agricore.iot.infrastructure.messaging.OutboxPublisher;
import com.agricore.iot.infrastructure.messaging.OutboxRetryProperties;
...
class IotOutboxPublisherIntegrationTest {
    ...
    void poisonEventIsQuarantinedWithoutStarvingHealthyEvent() throws InterruptedException {
        ...
        OutboxEventEntity poison = OutboxEventEntity.create(
                poisonId, "SensorReading", UUID.randomUUID().toString(), "Poison.v1",
                "agricore.iot.events", "{\"eventId\":\"" + poisonId + "\"}"
        );
        outboxRepository.saveAndFlush(poison);
        Thread.sleep(2);
        UUID healthyId = UUID.randomUUID();
        OutboxEventEntity healthy = OutboxEventEntity.create(
                healthyId, "SensorReading", UUID.randomUUID().toString(), "Healthy.v1",
                "agricore.iot.events", "{\"eventId\":\"" + healthyId + "\"}"
        );
    ...
    void deferredPoisonBatchDoesNotStarveTheNextHealthyPage() throws InterruptedException {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            poisonEvents.add(OutboxEventEntity.create(
                    eventId, "SensorReading", UUID.randomUUID().toString(), "Poison.v1",
                    "agricore.iot.events", "{\"eventId\":\"" + eventId + "\"}"
            ));
        }

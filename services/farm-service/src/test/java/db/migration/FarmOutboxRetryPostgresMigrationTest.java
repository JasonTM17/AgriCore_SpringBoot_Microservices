...
@Testcontainers
class FarmOutboxRetryPostgresMigrationTest {

    @Container
    private static final PostgreSQLContainer<?> POSTGRES = new PostgreSQLContainer<>("postgres:16-alpine")
            .withDatabaseName("agricore_farm_outbox")
    ...
    private static void createVersionEightOutboxTable(UUID legacyEventId) throws Exception {
        ...
        try (Connection connection = POSTGRES.createConnection("");
             var insert = connection.prepareStatement("""
                     ...
                     ) VALUES (?, 'Farm', 'legacy-farm', 'FarmCreated.v1', 'farm.events', '{}', ?)
                     """)) {
    ...
    private static void insertEvent(
        ...
        try (var insert = connection.prepareStatement("""
                ...
                ) VALUES (?, 'Farm', ?, 'FarmUpdated.v1', 'farm.events', '{}', ?, ?, ?, ?)
                """)) {
    ...
    private static void assertRetryQueueIndex(Connection connection) throws Exception {
        assertThat(indexDefinition(connection, "idx_farm_outbox_retry_queue"))
                .contains("(created_at, next_attempt_at)")
                .contains("WHERE ((published_at IS NULL) AND (quarantined_at IS NULL))");
        assertThat(indexIsValid(connection, "idx_farm_outbox_retry_queue")).isTrue();
    }

    private static void assertQuarantineIndex(Connection connection) throws Exception {
        assertThat(indexDefinition(connection, "idx_farm_outbox_quarantine"))
                .contains("(quarantined_at)")
                .contains("WHERE (quarantined_at IS NOT NULL)");
        assertThat(indexIsValid(connection, "idx_farm_outbox_quarantine")).isTrue();
    }

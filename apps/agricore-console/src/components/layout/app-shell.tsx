...
type NavigationItem = ReturnType<typeof visibleNavItems>[number];
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink md:grid md:grid-cols-[264px_1fr]">
      <aside
        className="hidden bg-forest-900 text-white md:block"
        aria-label="Điều hướng chính"
      >
        <NavigationPanel items={items} pathname={pathname} />
      </aside>
      ...
      {mobileOpen ? (
        <>
          <aside
            id="mobile-nav"
            className="fixed inset-y-0 left-0 z-40 w-[264px] max-w-[85vw] bg-forest-900 text-white md:hidden"
